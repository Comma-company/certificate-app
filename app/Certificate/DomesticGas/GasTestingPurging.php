...
class GasTestingPurging{
    public static function createPdf($certificate)
    {
        ...
        $html = view('dashboard.form.template.domestic_gas.Gas_Testing_Purging.index', [
            'data' => $data,
            'formData' =>   $formData,
        ...
        if ($cert_attachments->count() > 0) {
            $invoice->AddPage('L');
            $page_2 = view('dashboard.form.template.domestic_gas.Gas_Testing_Purging.note', [
                'data' => $certificate,
                'formData' => $formData,
                'cert_attachments' =>$cert_attachments,
